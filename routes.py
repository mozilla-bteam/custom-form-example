...
@route.route('/')
def hello():
    url = config.URL + '/auth.cgi?callback=' + config.SITE_URL + \
        '/callback&description=customforms'if config.LOGIN_METHOD else '/login'
...
def callback():
    if request.method == 'POST':
        ...
        client_api_key = User.query.filter_by(
            rand_id=request.args.get('callback_result'),
            client_api_login=request.args.get('client_api_login')).first().client_api_key
        if(client_api_key):
